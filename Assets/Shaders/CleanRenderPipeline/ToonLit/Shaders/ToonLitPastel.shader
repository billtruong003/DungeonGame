Shader "CleanRender/ToonLitPastel"
{
    Properties
    {
        ...
        [Header(Pastel)]
        _MinBrightness("Min Brightness", Range(0.05, 0.6)) = 0.35
        _PastelSoftness("Pastel Softness", Range(0.0, 0.5)) = 0.15

        [Header(Toon Shading)]
        _ShadowColor("Shadow Color", Color) = (0.55, 0.5, 0.65, 1)
        ...
        [Header(Emissive Mask)]
        [Toggle(_EMISSIVE_MASK)] _UseEmissiveMask("Use Emissive Mask", Float) = 0
    ...
    SubShader
    {
        ...
        half3 PastelFloor(half3 color, half minBrightness, half softness)
        {
            half luminance = dot(color, half3(0.2126h, 0.7152h, 0.0722h));
            half3 floorColor = max(color, half3(minBrightness, minBrightness, minBrightness));
            half lift = saturate((minBrightness - luminance) / max(minBrightness, 1e-4h));
            half3 lifted = lerp(color, floorColor, lift);
            half3 avgColor = dot(lifted, half3(0.333h, 0.333h, 0.334h)).xxx;
            half desatAmount = softness * lift;
            return lerp(lifted, lerp(lifted, avgColor, 0.4h), desatAmount);
        }
        ...
        Pass
        {
            ...
            HLSLPROGRAM
            #pragma vertex PastelVert
            #pragma fragment PastelFrag
            ...
            Varyings PastelVert(Attributes input)
            {
            ...
            half4 PastelFrag(Varyings input) : SV_Target
            {
                UNITY_SETUP_INSTANCE_ID(input);
                ...
                half3 diffuse = albedo.rgb * lerp(_ShadowColor.rgb, (half3)mainLight.color, cel);
                ...
                half3 additionalLights = half3(0.0h, 0.0h, 0.0h);
                #if defined(_ADDITIONAL_LIGHTS)
                {
                    ...
                    for (uint i = 0u; i < lightCount; i++)
                    {
                        ...
                        additionalLights += albedo.rgb * (half3)addLight.color * addCel;
                    }
                }
                #endif
                ...
                half3 finalColor = diffuse + gi + additionalLights + rim;

                finalColor = PastelFloor(finalColor, _MinBrightness, _PastelSoftness);
        ...
        Pass
        {
            ...
            half4 MetaFrag(Varyings input) : SV_Target
            {
                ...
                metaInput.Albedo = base.rgb * _BaseColor.rgb;
