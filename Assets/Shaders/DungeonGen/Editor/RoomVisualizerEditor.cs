...
namespace DungeonSystem.Editor
{
    ...
    public class RoomVisualizerEditor : UnityEditor.Editor
    {
        ...
        public static void DrawRoomGizmos(RoomInstance room, GizmoType gizmoType)
        {
            ...
            float cellSize = 16f; // Could be read from config if available

            // Try to find the actual cell size from parent DungeonGenerator
            var gen = room.GetComponentInParent<DungeonGenerator>();
            if (gen != null && gen.config != null)
                cellSize = gen.config.cellSize;
        ...
        private static void DrawRoomBounds(RoomInstance room, bool isSelected, float cellSize)
        {
            ...
            Vector3 size = new Vector3(
                room.widthInCells * cellSize,
                1f,
                room.heightInCells * cellSize);
        ...
        private static void DrawSockets(RoomInstance room, bool isSelected)
        {
            ...
            foreach (var socket in room.sockets)
            {
                ...
                Color socketColor = socket.CurrentState switch
                {
                    ...
                    DoorState.Walled => new Color(0.5f, 0.5f, 0.5f),
                    DoorState.Locked => Color.yellow,
                    DoorState.Hidden => new Color(0.6f, 0.2f, 0.6f),
                    _ => Color.white
                };
        ...
        private static Color GetTypeColor(RoomType type)
        {
            return type switch
            {
                ...
                RoomType.MiniBoss    => new Color(0.8f, 0.4f, 0.1f),
                RoomType.Combat      => new Color(0.5f, 0.5f, 0.5f),
                RoomType.Loot        => new Color(0.8f, 0.7f, 0.1f),
                RoomType.Puzzle      => new Color(0.1f, 0.5f, 0.8f),
                RoomType.Shop        => new Color(0.3f, 0.7f, 0.3f),
                RoomType.SafeRoom    => new Color(0.3f, 0.6f, 0.8f),
                RoomType.SecretRoom  => new Color(0.6f, 0.1f, 0.6f),
                RoomType.Trap        => new Color(0.7f, 0.4f, 0.1f),
                RoomType.Corridor    => new Color(0.4f, 0.4f, 0.4f),
                _ => Color.gray
            };
        }
    }
