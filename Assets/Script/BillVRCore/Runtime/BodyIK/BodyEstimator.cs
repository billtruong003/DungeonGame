    ...
    public class BodyEstimator : MonoBehaviour
    {
        ...
        private Quaternion EstimateHipRotation()
        {
            ...
            if (leftHandTarget != null && rightHandTarget != null)
            {
                ...
                {
                    float handSpread = Vector3.Distance(leftHandTarget.position, rightHandTarget.position);
                    float handInfluence = Mathf.Clamp01(handSpread / Mathf.Max(hipWidth + shoulderWidth, 0.01f));
                    headForward = Vector3.Slerp(headForward, handsMidForward.normalized, 0.3f * handInfluence);
                }
            }
