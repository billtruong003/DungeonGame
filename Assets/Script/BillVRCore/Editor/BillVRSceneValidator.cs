...
{
    public class BillVRSceneValidator : EditorWindow
    {
        private Vector2 _scroll;

        [MenuItem("BillVR/Validate Scene", priority = 43)]
        public static void ShowWindow()
        {
            var window = GetWindow<BillVRSceneValidator>("Scene Validator");
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField("BillVR Scene Validation", EditorStyles.boldLabel);
            EditorGUILayout.Space(8);

            int issues = 0;
            issues += CheckComponent<BillVRBootstrap>("BillVRBootstrap");
        ...
        {
            int issues = 0;
            foreach (var (name, index) in BillVRLayerSetup.RequiredLayers)
            {
