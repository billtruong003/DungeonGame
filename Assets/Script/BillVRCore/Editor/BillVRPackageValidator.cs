...
{
    public static class BillVRPackageValidator
    {
        ...
        private const string RuntimeAsmdefGuid = "BillVR.Runtime";
        ...
        public static void ApplyRagdollIntegration(bool hasRagdoll, string ra2AssemblyName)
        {
            ...
            if (hasRagdoll && !string.IsNullOrEmpty(ra2AssemblyName))
            {
                ...
                {
                    Debug.LogWarning(
                        "[BillVR] Ragdoll Animator 2 found but it compiles into Assembly-CSharp (no .asmdef). " +
                        "BillVR.Runtime.asmdef cannot reference Assembly-CSharp. " +
        ...
        private static void UpdateScriptingDefine(bool hasRagdoll)
        {
            ...
            if (targetGroup == BuildTargetGroup.Unknown) targetGroup = BuildTargetGroup.Standalone;
            ...
            if (hasRagdoll && !hasDefine)
            {
                ...
                Debug.Log("[BillVR] Added VRCORE_HAS_RAGDOLL scripting define.");
            }
            else if (!hasRagdoll && hasDefine)
            {
                defineList.Remove(RagdollDefine);
                PlayerSettings.SetScriptingDefineSymbols(namedTarget, string.Join(";", defineList));
                Debug.Log("[BillVR] Removed VRCORE_HAS_RAGDOLL scripting define.");
            }
        }

        private static string FindVRCoreRuntimeAsmdef()
        {
            string[] guids = AssetDatabase.FindAssets("BillVR.Runtime t:asmdef");
            foreach (string guid in guids)
            {
                string path = AssetDatabase.GUIDToAssetPath(guid);
                if (path.EndsWith("BillVR.Runtime.asmdef"))
                    return path;
            }
            ...
            foreach (string guid in allAsmdefs)
            {
                ...
                try
                {
                    ...
                    if (json.Contains("\"name\": \"BillVR.Runtime\"") || json.Contains("\"name\":\"BillVR.Runtime\""))
                        return path;
                }
                catch { }
            }
        ...
        private static void AddAssemblyReference(string assemblyName)
        {
            ...
            if (asmdefPath == null)
            {
                Debug.LogWarning("[BillVR] Could not find VRCore.Runtime.asmdef to add RA2 reference.");
                return;
            }
            ...
            {
                Debug.Log($"[BillVR] VRCore.Runtime.asmdef already references '{assemblyName}'.");
                return;
            }
            ...
            if (!refs.Contains(assemblyName))
            {
                ...
                Debug.Log($"[BillVR] Added '{assemblyName}' reference to VRCore.Runtime.asmdef for Ragdoll Animator 2 integration.");
            }
        }
        ...
        private static void RemoveAllRA2References()
        {
            ...
            if (removed > 0)
            {
                ...
                File.WriteAllText(asmdefPath, newJson);
                AssetDatabase.Refresh();
                Debug.Log("[BillVR] Removed RA2 references from VRCore.Runtime.asmdef.");
            }
        }
        ...
        public static void InstallPackage(string packageId)
        {
            ...
            Debug.Log($"[BillVR] Installing {packageId}...");
        }
