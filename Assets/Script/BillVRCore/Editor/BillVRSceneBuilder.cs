...
using BillVRCore.Locomotion;

namespace BillVRCore.Editor
{
    public static class BillVRSceneBuilder
    {
        [MenuItem("BillVR/Apply VR Performance Settings", priority = 42)]
        public static void ApplyVRPerformanceSettings()
        {
            Application.targetFrameRate = 120;
            QualitySettings.vSyncCount = 0;
            Time.fixedDeltaTime = 0.01111f;
            Debug.Log("[BillVR] Performance: 120fps target, vSync off, physics 90Hz.");
        }
        ...
        public static GameObject BuildPlayerRig()
        {
            ...
            if (existing != null)
            {
                Debug.LogWarning("[BillVR] Player rig already exists in scene.");
                return existing.gameObject;
            }

            ApplyVRPerformanceSettings();

            var root = new GameObject("[BillVR] Player");
            Undo.RegisterCreatedObjectUndo(root, "Create VRCore Player");
            root.transform.position = Vector3.zero;

            int playerBodyLayer = GetLayer("PlayerBody");
            if (playerBodyLayer >= 0) root.layer = playerBodyLayer;

            root.AddComponent<BillVRBootstrap>();
            ...
            root.AddComponent<BillVRCore.DebugTools.BillVRDebugOverlay>();
            ...
            var groundMaskProp = bodyObj.FindProperty("groundMask");
            if (groundMaskProp != null)
                groundMaskProp.intValue = BuildGroundMask();
            bodyObj.ApplyModifiedProperties();

            CreateHighlightMaterial();

            Selection.activeGameObject = root;
            Debug.Log("[BillVR] Player rig created with full locomotion.");
            return root;
        }
        ...
        private static GameObject CreateHand(string name, HandSide side, Transform followTarget, Transform parent)
        {
            ...
            var grabHandler = handGo.AddComponent<GrabHandler>();
            handGo.AddComponent<HandHighlighter>();
            handGo.AddComponent<HandHaptics>();
            var distGrabber = handGo.AddComponent<DistanceGrabber>();
            ...
            // Set HandPhysics layer on entire hand hierarchy (Palm, HandVisual included)
            int handLayer = GetLayer("HandPhysics");
            if (handLayer >= 0)
                SetLayerRecursive(handGo, handLayer);

            // Configure grab detection to only scan Grabbable layer
            int grabbableLayer = GetLayer("Grabbable");
            if (grabbableLayer >= 0)
            {
                int grabMask = 1 << grabbableLayer;
                ConfigureLayerMask(grabHandler, "grabLayers", grabMask);
                ConfigureLayerMask(distGrabber, "grabLayers", grabMask);
            }
        ...
        public static void CreateTestGrabbables(int count = 5)
        {
            ...
            Debug.Log($"[BillVR] Created {count} basic grabbables.");
        }

        [MenuItem("BillVR/Create Diverse Test Objects", priority = 22)]
        public static void CreateDiverseTestObjects()
        {
            float z = 0.6f;

            // Row 1: Mass / weight variety (Hold mode — default)
            ...
            // Row 2: Grab modes & types
            CreateToggleGrabObj(new Vector3(-1f, 1f, z - 0.35f));
            CreatePullToHandObj(new Vector3(-0.6f, 1f, z - 0.35f));
            CreateInstantGrabObj(new Vector3(-0.2f, 1f, z - 0.35f));
            CreateMaintainOffsetObj(new Vector3(0.2f, 1f, z - 0.35f));

            // Row 3: Physics gadgets
            ...
            Debug.Log("[BillVR] Created diverse test objects.");
        }
        ...
        private static void CreateToggleGrabObj(Vector3 pos)
        {
            var cube = GameObject.CreatePrimitive(PrimitiveType.Cube);
            cube.name = "Grab_Toggle";
            cube.transform.position = pos;
            cube.transform.localScale = Vector3.one * 0.1f;
            SetGrabbableLayer(cube);
            ...
            var rb = cube.AddComponent<Rigidbody>();
            rb.mass = 0.3f;
            var grab = cube.AddComponent<Grabbable>();
            grab.SetGrabMode(GrabMode.Toggle);

            var mat = new Material(Shader.Find("Universal Render Pipeline/Lit"));
            mat.color = new Color(1f, 0.5f, 0f);
            cube.GetComponent<MeshRenderer>().sharedMaterial = mat;
            Undo.RegisterCreatedObjectUndo(cube, "Toggle Grab");
        }

        private static void CreatePullToHandObj(Vector3 pos)
        {
            var sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);
            sphere.name = "Grab_PullToHand";
            sphere.transform.position = pos;
            ...
            SetSerializedGrabbableField(grab, "grabType", (int)GrabType.GrabbableToHand);

            var mat = new Material(Shader.Find("Universal Render Pipeline/Lit"));
            mat.color = new Color(0.2f, 1f, 0.6f);
            sphere.GetComponent<MeshRenderer>().sharedMaterial = mat;
            Undo.RegisterCreatedObjectUndo(sphere, "Pull To Hand");
        }

        private static void CreateInstantGrabObj(Vector3 pos)
        {
            var cube = GameObject.CreatePrimitive(PrimitiveType.Cube);
            cube.name = "Grab_Instant";
            cube.transform.position = pos;
            ...
            rb.mass = 0.1f;
            var grab = cube.AddComponent<Grabbable>();
            SetSerializedGrabbableField(grab, "grabType", (int)GrabType.Instant);

            var mat = new Material(Shader.Find("Universal Render Pipeline/Lit"));
            mat.color = new Color(1f, 1f, 0.2f);
            cube.GetComponent<MeshRenderer>().sharedMaterial = mat;
            Undo.RegisterCreatedObjectUndo(cube, "Instant Grab");
        }

        private static void CreateMaintainOffsetObj(Vector3 pos)
        {
            var cube = GameObject.CreatePrimitive(PrimitiveType.Cube);
            cube.name = "Grab_MaintainOffset";
            cube.transform.position = pos;
            cube.transform.localScale = new Vector3(0.12f, 0.06f, 0.06f);
            SetGrabbableLayer(cube);

            var rb = cube.AddComponent<Rigidbody>();
            rb.mass = 0.5f;
            var grab = cube.AddComponent<Grabbable>();
            SetSerializedGrabbableField(grab, "maintainGrabOffset", true);

            var mat = new Material(Shader.Find("Universal Render Pipeline/Lit"));
            mat.color = new Color(0.6f, 0.2f, 1f);
            cube.GetComponent<MeshRenderer>().sharedMaterial = mat;
            Undo.RegisterCreatedObjectUndo(cube, "Maintain Offset");
        }
        ...
        private static void CreatePhysicsLeverObj(Vector3 pos)
        {
            ...
            rb.useGravity = false;
            rb.mass = 0.3f;
            handle.AddComponent<Grabbable>();
            handle.AddComponent<PhysicsLever>();
        ...
        private static void CreatePhysicsDialObj(Vector3 pos)
        {
            ...
            rb.useGravity = false;
            rb.mass = 0.2f;
            dial.AddComponent<Grabbable>();
            dial.AddComponent<PhysicsDial>();
        ...
        private static void CreatePhysicsSliderObj(Vector3 pos)
        {
            ...
            rb.mass = 0.2f;
            handle.AddComponent<Grabbable>();
            handle.AddComponent<PhysicsSlider>();
        ...
        private static void CreateClimbWall(Vector3 pos)
        {
            ...
            SetGrabbableLayer(wall);

            // Climb wall is static — Grabbable requires Rigidbody, so make it kinematic
            ...
            grab.SetDisableGrabTracking(true);
        ...
        private static void CreateHighlightMaterial()
        {
            ...
            var shader = Shader.Find("BillVR/OutlineHighlight");
        ...
        private static int GetLayer(string name)
        {
            ...
            if (layer == -1)
                Debug.LogWarning($"[BillVR] Layer '{name}' not found. Run BillVR > Setup Wizard to create layers.");
            return layer;
        }
        ...
        private static LayerMask BuildGroundMask()
        {
            int exclude = 0;
            string[] names = { "PlayerBody", "HandPhysics", "GroundCheck", "BodyIK", "InventorySlot", "RagdollDummy" };
        ...
        private static void ConfigureLayerMask(Component comp, string fieldName, int mask)
        {
        ...
        private static void SetSerializedGrabbableField(Grabbable grab, string fieldName, int value)
        {
        ...
        private static void SetSerializedGrabbableField(Grabbable grab, string fieldName, bool value)
        {
            var so = new SerializedObject(grab);
            var prop = so.FindProperty(fieldName);
            if (prop != null)
