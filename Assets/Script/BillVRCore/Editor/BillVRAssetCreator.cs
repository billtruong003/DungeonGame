...
using BillVRCore.Inventory;

namespace BillVRCore.Editor
{
    public static class BillVRAssetCreator
    {
        private const string ConfigPath = "Assets/VRCore/Data";

        [MenuItem("BillVR/Create Default Configs", priority = 42)]
        public static void CreateAllDefaults()
        {
            ...
            Debug.Log("[BillVR] Default config assets created in Assets/VRCore/Data/");
        }
        ...
        private static void CreatePose(string name, float thumb, float index, float middle, float ring, float pinky)
        {
            ...
            if (AssetDatabase.LoadAssetAtPath<HandPoseAsset>(path) != null) return;
