Shader "CleanRender/ToonTerrain"
{
    ...
    SubShader
    {
        ...
        {
            return w * rcp(max(dot(w, 1.0h), 1e-4h));
        }

        half SampleHoleAlpha(float2 uv)
        {
            return SAMPLE_TEXTURE2D(_HoleMap, sampler_HoleMap, TRANSFORM_TEX(uv, _HoleMap)).r;
        }

        // Computes terrain blend weights from height, normal, and optional splat.
        // Factored out so ForwardLit and Meta can share the logic.
        half4 ComputeTerrainWeights(float height, half3 N, float2 uv)
        {
            half blendScale = _BlendSharpness * 0.1h;
            half w0 = saturate(1.0h - saturate((height - _HeightLow) * blendScale));
            half w2 = saturate((height - _HeightMid) * blendScale);
            half w1 = max(1.0h - w0 - w2, 0.0h);

            half cliffInv = rcp(max(1.0h - _CliffAngle + 0.01h, 1e-4h));
            half cliffMask = 1.0h - saturate((N.y - _CliffAngle) * cliffInv);
            half splatCliffOverride = 0.0h;

            #ifdef _USE_SPLATMAP
            {
                half4 splat = SAMPLE_TEXTURE2D(_SplatMap, sampler_SplatMap, TRANSFORM_TEX(uv, _SplatMap));

                // Single-pass sharpen + normalize (avoids double normalize)
                half4 sharpSplat = pow(splat, _SplatSharpness);
                sharpSplat *= rcp(max(dot(sharpSplat, 1.0h), 1e-4h));
            ...
            half flatScale = (1.0h - cliffMask) * rcp(max(flatSum, 1e-4h));

            return NormalizeWeights(half4(w0 * flatScale, w1 * flatScale, w2 * flatScale, cliffMask));
        }

        // Sample all 4 layers and blend by weights. Returns albedo.
        half3 SampleTerrainAlbedo(float3 positionWS, half3 N, half4 weights)
        {
            float2 worldUV = positionWS.xz * _TexScale;
            ...
            TriplanarUV tp = ComputeTriplanarUV(positionWS, N, _TriplanarScale, _TriplanarSharpness);
        ...
        Pass
        {
            ...
            half4 TerrainFrag(Varyings input) : SV_Target
            {
                ...
                UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
                    ...
                    clip(SampleHoleAlpha(input.uv) - _HoleThreshold);
                #endif

                half3 N = normalize(input.normalWS);
                float height = input.positionWS.y - _HeightOffset;

                half4 weights = ComputeTerrainWeights(height, N, input.uv);
                half3 albedo  = SampleTerrainAlbedo(input.positionWS, N, weights);
                ...
                return half4(MixFog(lit.color, input.fogFactor), 1.0h);
            }
            ENDHLSL
        }
        ...
        // ════════════════════════════════════════════════════════════════
        //  Pass 4 – Meta  (lightmap baking / GI)
        // ════════════════════════════════════════════════════════════════
        Pass
        {
            Name "Meta"
            ...
            HLSLPROGRAM
            #pragma vertex TerrainMetaVert
            #pragma fragment TerrainMetaFrag
            ...
            MetaVary TerrainMetaVert(MetaAttr input)
            {
            ...
            half4 TerrainMetaFrag(MetaVary input) : SV_Target
            {
                #if defined(_USE_HOLEMAP) || defined(_ALPHATEST_ON)
                    clip(SampleHoleAlpha(input.uv) - _HoleThreshold);
                #endif
