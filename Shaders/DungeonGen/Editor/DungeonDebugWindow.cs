...
{
    /// <summary>
    /// Draws debug gizmos in Scene View showing the dungeon graph, layout grid,
    /// room types, and corridor paths.
    /// </summary>
    public class DungeonDebugWindow : EditorWindow
    {
        ...
        private static void OnSceneGUI(SceneView sceneView)
        {
            ...
            foreach (var floor in generator.FloorResults)
            {
                ...
                if (_showGrid)
                    DrawGridCells(floor.Layout, cellSize, yOffset);

                if (_showGraph)
                    DrawGraphEdges(floor, cellSize, yOffset);

                if (_showLabels)
                    DrawRoomLabels(floor, cellSize, yOffset);
            }
        }
        ...
        private static void DrawGridCells(FloorLayout layout, float cellSize, float yOffset)
        {
            foreach (var cell in layout.OccupiedCells)
            {
                ...
                Color color = isRoom
                    ? new Color(0.3f, 0.6f, 0.3f, 0.15f)
                    : new Color(0.5f, 0.5f, 0.2f, 0.1f); // Corridor
        ...
        private static void DrawGraphEdges(FloorResult floor, float cellSize, float yOffset)
        {
            foreach (var edge in floor.Graph.Edges)
            {
                ...
                else if (edge.IsSecret)
                {
                    Handles.color = new Color(0.7f, 0f, 0.7f, 0.5f);
        ...
        {
            return new Vector3(
                gridPos.x * cellSize + w * cellSize * 0.5f,
                yOffset + 0.1f,
                gridPos.y * cellSize + h * cellSize * 0.5f);
        }
        ...
        private static Color GetRoomTypeColor(RoomType type)
        {
            return type switch
            {
                ...
                RoomType.Loot        => Color.yellow,
                RoomType.Puzzle      => Color.cyan,
                RoomType.Shop        => new Color(0.5f, 1f, 0.5f),
                RoomType.SafeRoom    => new Color(0.5f, 0.8f, 1f),
                RoomType.SecretRoom  => new Color(0.8f, 0.3f, 0.8f),
                RoomType.Trap        => new Color(1f, 0.6f, 0.2f),
